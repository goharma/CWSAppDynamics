...
while [ 1 ]; do
	...
	iostat 1 3 | awk -v OFS="" '
		...
		END {
			...
			print "name=Hardware Resources|Disks|KB read/sec,value=", kbps;
		}
	'
...
(netstat -in ; sleep $SAMPLE; netstat -in) |
	awk -v interval=$SAMPLE -v OFS="" '
		...
		END {
			...
			print "name=Hardware Resources|Network|Incoming KB/sec,value=", NET_PACKETS_IN;
			print "name=Hardware Resources|Network|Outgoing KB/sec,value=", NET_PACKETS_OUT
		}
	'
